/*
* dutyFree.utility is a helper module
* containing common functions/variables.
*
*/

var dutyFree = dutyFree || {};

dutyFree.utility = (function () {
    var instance = {
    };
    ...
    instance.getImgUrl = function (imgId) {
        return "/api/img/" + imgId;
    };

    instance.addToCart = function (userId, $http, $rootScope, $scope, productId) {
        if (userId) {
            var url = _basePath + "/api/preference/addProduct?userId=" + userId + "&productId=" + productId;
            $http.post(url);
            $rootScope.$broadcast("plusCount", 1);
            $scope.showModal();
        }
        else {
            var url = _basePath + "/api/session/ProductId?value=" + productId;
